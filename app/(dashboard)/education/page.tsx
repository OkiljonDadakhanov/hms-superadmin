...
export default function EducationPage() {
  ...
  const categories = Array.from(new Set(educationContents.map((content) => content.category)))
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {paginatedContents.map((content) => {
          const assignedPatients = content.assignedTo
            ? content.assignedTo.map((id) => patients.find((p) => p.id === id)?.name).filter(Boolean)
            : []
          ...
          return (
            <Card key={content.id} className="overflow-hidden">
              ...
              <CardFooter className="flex justify-between pt-2">
                <Button variant="outline" size="sm" onClick={() => handleViewContent(content.id)}>
                  View
                </Button>
      ...
      <Dialog open={isViewContentOpen} onOpenChange={setIsViewContentOpen}>
        <DialogContent className="sm:max-w-[800px] max-h-[80vh] overflow-y-auto">
          ...
          {selectedContent && <EducationContentView contentId={selectedContent} />}
        </DialogContent>
      </Dialog>
      ...
      <AlertDialog open={isDeleteContentOpen} onOpenChange={setIsDeleteContentOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Are you sure?</AlertDialogTitle>
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the education content.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
